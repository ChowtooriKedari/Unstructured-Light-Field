...
def normalize_depth_map(depth_map):
    ...
    normalized_depth = (depth_map - min_val) / (max_val - min_val) * 255
...
for depth in depth_maps:
    normalized_depth = normalize_depth_map(depth)
    plt.imshow(normalized_depth, cmap='gray')
    plt.show()
    cv2.imwrite(f'Main3_depth{count+1}.jpg', normalized_depth)
    count += 1
