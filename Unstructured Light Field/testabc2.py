...
def process_images(metadata_path, calibration_file, stereo_params_file, baseline, focus_depth):
    ...
    refocused_img = refocus_image(images, depth_maps, focus_depth)
    
    plt.imshow(cv2.cvtColor(all_focus_img, cv2.COLOR_BGR2RGB))
    plt.title("All-in-Focus Image")
    plt.show()
    cv2.imwrite('all_in_focus.jpg', all_focus_img)
    
    plt.imshow(cv2.cvtColor(refocused_img, cv2.COLOR_BGR2RGB))
    plt.title(f"Refocused Image at Depth {focus_depth}")
    plt.show()
    cv2.imwrite(f'refocused_at_depth_{focus_depth}.jpg', refocused_img)
    
    return all_focus_img, refocused_img
...
focus_depth = 1000  # Desired focus depth in the same units as the depth map (e.g., mm)

all_in_focus_image, refocused_img = process_images(metadata_path, calibration_file, stereo_params_file, baseline, focus_depth)
