...
def sqaureAperture(a,u,v):
    if(np.abs(u)<=a/2 and np.abs(v)<=a/2):
        return True
    return False 
    
def circularAperture(a,u,v):
    if(math.sqrt(math.pow(u,2)+math.pow(v,2))<=a/2):
        return True
    return False
...
def interp(I,u,v,depth):
    ...
    im=interpolate.interp2d(p,q,I)
    return im(p+(depth*u),q-(depth*v))
...
def depthRefocus(lightfield,depth,a=1,aperture='max'):
    ...
    for u in range(len(U)):
        for v in range(len(V)):
            ...
            if (aperture == 'square' and sqaureAperture(U[u], V[v], maxUV) == False) or (aperture == 'circle' and circularAperture(U[u], V[v], maxUV) == False) :
                continue
...
plot = plt.figure()
image_1 = depthRefocus(L, -1.5,1,'Square')
plot.add_subplot(2,3,1)
plt.title('Depth=-1.5')
plt.imshow(image_1)
image_2 = depthRefocus(L, -1.0,1,'Square')
plot.add_subplot(2,3,2)
plt.title('Depth=-1.0')
plt.imshow(image_2)
image_3 = depthRefocus(L, -0.5,1,'Square')
plot.add_subplot(2,3,3)
plt.title('Depth=-0.5')
plt.imshow(image_3)
image_4 = depthRefocus(L,0,1,'Square')
plot.add_subplot(2,3,4)
plt.title('Depth=0')
plt.imshow(image_4)
image_5 = depthRefocus(L, 0.5,1,'Square')
plot.add_subplot(2,3,5)
plt.title('Depth=0.5')
plt.imshow(image_5)
image_6 = depthRefocus(L, 1.0,1,'Square')
...
pltname='Bonus1_focusImageNew.exr'
pltname1='Bonus1_DepthMapNew.exr'
