...
#     stereo = cv2.StereoBM_create(numDisparities=192, blockSize=125)  # Adjusted numDisparities and blockSize
...
def compute_disparity_map_bm(img_left, img_right):
    gray_left = cv2.cvtColor(img_left, cv2.COLOR_BGR2GRAY)
    gray_right = cv2.cvtColor(img_right, cv2.COLOR_BGR2GRAY) 
    stereo = cv2.StereoBM_create(numDisparities=192, blockSize=15)  # Adjusted numDisparities and blockSize
    disparity = stereo.compute(gray_left, gray_right).astype(np.float64) / 16.0
    return disparity
...
def refocus_image(images, depth_maps, focus_depth):
    refocused_images = []
    for cam_images, depth_map in zip(images, depth_maps):
        refocused_image = np.zeros_like(cam_images[0])
        for img in cam_images:
            mask = np.abs(depth_map - focus_depth) < 1e-1  # Adjust the threshold as needed
            refocused_image[mask] = img[mask]
        refocused_images.append(refocused_image)
    return refocused_images
...
def process_images(metadata_path, calibration_file, stereo_params_file, baseline, focus_depth):
    ...
    refocused_images = refocus_image(images, depth_maps, focus_depth)
    
    count = 0
    for refocused_image in refocused_images:
        plt.imshow(cv2.cvtColor(refocused_image, cv2.COLOR_BGR2RGB))
        plt.title(f"Refocused Image {count + 1}")
        plt.show()
        cv2.imwrite(f'Main_6_refocused{count + 1}.jpg', refocused_image)
        count += 1
...
stereo_params_file = 'stereo_params.json'
baseline = 50  # Distance between the cameras in the same units as the focal length (e.g., mm)
focus_depth = 1  # Example focus depth, adjust as needed

refocused_images = process_images(metadata_path, calibration_file, stereo_params_file, baseline, focus_depth)
