...
def compute_disparity_map_bm(img_left, img_right):
    ...
    numDisparities = 160  # Should be divisible by 16
...
def refocus_image(images, depth_maps, focus_depth):
    refocused_img = np.zeros_like(images[0][0], dtype=np.float64)
    count = np.zeros_like(images[0][0], dtype=np.float64)
    
    for idx, (cam_images, depth_map) in enumerate(zip(images, depth_maps)):
        mask = np.abs(depth_map - focus_depth) < (0.1 * focus_depth)  # Select pixels close to focus depth
        for img in cam_images:
...
def process_images(metadata_path, calibration_file, stereo_params_file, baseline, focus_depth):
...
depth_maps = process_images(metadata_path, calibration_file, stereo_params_file, baseline, focus_depth)
...
def refocus_image(image, depth_map, focus_depth, blur_kernel_size=21):
    ...
    mask = np.repeat(mask[:, :, np.newaxis], 3, axis=2)  # Expand mask to 3 dimensions
    
    # Combine blurred and sharp images based on mask
    refocused_image[~mask] = blurred_image[~mask]
...
# Generate refocused images for each set of 4 images using their corresponding depth map
refocused_images = []
for i in range(4):
    refocused_set = [refocus_image(images[j], depth_images[i], focus_depth) for j in range(i*4, (i+1)*4)]
    refocused_images.extend(refocused_set)
    
    # # Display each refocused image before merging
    # for item in refocused_set:
    #     plt.imshow(cv2.cvtColor(item, cv2.COLOR_BGR2RGB))
    #     plt.title(f'Refocused Image Set {i+1}')
    #     plt.show()
...
# Combine all refocused images into a single composite image
